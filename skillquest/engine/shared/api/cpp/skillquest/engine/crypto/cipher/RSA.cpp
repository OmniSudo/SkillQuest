#include "skillquest/engine/crypto/cipher/RSA.hpp"
#include <utility>
#include <openssl/rsa.h>
#include <openssl/pem.h>
#include <openssl/err.h>
#include "skillquest/engine/crypto/cipher/exception/RSAException.hpp"
...
namespace skillquest::crypto::cipher {
	...
	Keypair RSA::generate () {
		...
		std::vector< char > priKey = std::vector<char>(priLength + 1);
		std::vector< char > pubKey = std::vector<char>(priLength + 1);
		
		BIO_read( pri, priKey.data(), priLength );
		BIO_read( pub, pubKey.data(), pubLength );
		...
		return Keypair(
				std::string( pubKey.data(), pubLength ).substr( 31 ).substr( 0, pubLength - 31 - 30 ),
				std::string( priKey.data(), priLength ).substr( 32 ).substr( 0, priLength - 32 - 31 )
		);
	}
	...
	std::string RSA::encryptPublic ( std::string plaintext, Keypair keypair ) {
		...
		std::vector<char> encrypt = std::vector<char>(RSA_size( rsa ));
		int encryptLength;
		encryptLength = RSA_public_encrypt(
				plaintext.length(),
				reinterpret_cast<const unsigned char*>(plaintext.c_str()),
				( unsigned char* ) encrypt.data(), rsa, RSA_PKCS1_PADDING
		);
		RSA_free( rsa );
		if ( encryptLength == -1 ) error();
		return std::string( encrypt.data(), encryptLength );
	}
