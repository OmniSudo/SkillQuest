    ...
    public:
        ...
        explicit ChunkDataPacket(const json &data)
            : network::IPacket(data),
              _pos{
                data[ "pos" ]["x"].get< glm::detail::uint16 >(),
                data[ "pos" ]["y"].get< glm::detail::uint16 >(),
                data[ "pos" ]["z"].get< glm::detail::uint16 >()
              } {
        ...
        json serialize() const override {
            ...
            std::string converted = convert::base64::encode(
                std::string((char *) &formatted[0], formatted.size() * sizeof(std::size_t)));
